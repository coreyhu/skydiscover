...
class Evaluator:
        ...
        for attempt in range(self.config.max_retries + 1):
            ...
            try:
                if self.config.cascade_evaluation:
                    result = await self._cascade_evaluate(temp_path, budget_scope)
                else:
                    result = await self._run_stage(
                        self.evaluate_function, temp_path, budget_scope
                    )
    ...
    async def _run_stage(self, func, program_path: str, budget_scope: str) -> Any:
    ...
    async def _cascade_evaluate(
        self, program_path: str, budget_scope: str
    ) -> EvaluationResult:
        ...
        if not hasattr(module, "evaluate_stage1"):
            return self._normalize_result(
                await self._run_stage(self.evaluate_function, program_path, budget_scope)
            )

        # Stage 1
        try:
            stage1 = self._normalize_result(
                await self._run_stage(module.evaluate_stage1, program_path, budget_scope)
            )
        except asyncio.TimeoutError:
        ...
        try:
            stage2 = self._normalize_result(
                await self._run_stage(module.evaluate_stage2, program_path, budget_scope)
            )
        except asyncio.TimeoutError:
