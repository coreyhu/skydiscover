...
USE_TEST_IN_SCORE = os.getenv("ARC_EVAL_USE_TEST_FOR_SCORE", "0").lower() in (
    "1",
    "true",
    "yes",
)
...
def cell_accuracy_single(pred: np.ndarray, gt: np.ndarray) -> float:
    ...
    if pred.shape != gt.shape:
        ...
        if len(pred.shape) == len(gt.shape) == 2:
            ...
            shape_score = (
                row_match + col_match
            ) * 0.1  # up to 0.2 for correct dimensions
        return shape_score
    # Cell-level accuracy
    total_cells = gt.size
    if total_cells == 0:
...
def extract_failure_artifacts(diagnostics, pred=None, gt=None):
    ...
    if not diagnostics["size_match"]:
        ...
        if diagnostics["incorrect_indices"] and pred is not None and gt is not None:
            ...
            for r, c in wrong:
                diff_lines.append(
                    f"  [{r},{c}]: got {int(pred[r, c])}, expected {int(gt[r, c])}"
                )
        ...
        if os.path.isfile(solution_path):
            ...
            if solution is not None and "test" in task:
                ...
                for inp in test_inputs:
                    ...
                    if not isinstance(attempt_1, np.ndarray):
                        print(
                            f"transform_grid_attempt_1 did not return a numpy array (test)"
                        )
                        return EvaluationResult(
                            metrics={
                                "runs_successfully": 0.0,
                    ...
                    if not isinstance(attempt_2, np.ndarray):
                        print(
                            f"transform_grid_attempt_2 did not return a numpy array (test)"
                        )
                        return EvaluationResult(
                            metrics={
                                "runs_successfully": 0.0,
                ...
                for i, (test_pass, test_diagnostics) in enumerate(
                    ...
                    for attempt in test_diagnostics:
                        metrics[f"{example_name}_attempt_{attempt}"] = test_diagnostics[
                            attempt
                        ]["perfect_match"]
                    if test_pass == 0:
