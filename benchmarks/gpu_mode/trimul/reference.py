...
def check_implementation(data, submission_output, rtol=2e-2, atol=2e-2):
    ...
    try:
        ...
        if ref_output.shape != submission_output.shape:
            return (
                False,
                f"Shape mismatch: {ref_output.shape} vs {submission_output.shape}",
            )
        if torch.allclose(
            ref_output.float(), submission_output.float(), rtol=rtol, atol=atol
        ):
            return True, "Match"
        diff = torch.abs(ref_output.float() - submission_output.float())
        return (
            False,
            f"max_diff={diff.max().item():.6f}, avg_diff={diff.mean().item():.6f}",
        )
    finally:
        torch.backends.cuda.matmul.allow_tf32 = old_matmul
        torch.backends.cudnn.allow_tf32 = old_cudnn
