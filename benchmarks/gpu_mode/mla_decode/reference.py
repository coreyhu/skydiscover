...
class MLA(nn.Module):
    def __init__(self, config: Config):
        ...
        self.Q_proj_down = nn.Linear(
            self.dim, self.q_lora_rank, dtype=torch.bfloat16, bias=False
        )
...
def generate_input(batchsize, dim, dq, prefill, seed):
    ...
    x = torch.randn(
        (config.batch_size, 1, config.dim),
        dtype=torch.bfloat16,
        generator=gen,
        device="cuda",
    )
...
def check_implementation(data, submission_output, rtol=2e-2, atol=8e-3):
    ...
    good_kv, reasons_kv = _verbose_allclose(
        output_kv_cpu, expected_kv_cpu, rtol=rtol, atol=atol
    )
