...
class BestProgramEvaluator:
    ...
    def __init__(
        ...
        logger.info(
            f"Using {self.num_workers} parallel workers with thread-local evaluators"
        )
    ...
    def _get_evaluator(self) -> "FrontierCSEvaluator":
        ...
        if not hasattr(self._evaluator_local, "evaluator"):
            ...
            logger.debug(
                f"Created new evaluator for thread {threading.current_thread().name}"
            )
    ...
    def evaluate_program(self, problem_id: str, program_path: Path) -> Dict:
        ...
        try:
            ...
            logger.info(
                f"Code extracted from {program_path}, length: {len(code)} characters"
            )
            ...
            if result.status == EvaluationStatus.SUCCESS:
                ...
                logger.error(
                    f"Problem {problem_id}: Evaluation failed with status {result.status}"
                )
                return {
                    "problem_id": problem_id,
                    "program_path": str(program_path),
    ...
    def print_summary(self):
        ...
        compilation_error = [
            r for r in self.results if r.get("status") == "compilation_error"
        ]
        other_error = [
            r
        ...
        if successful:
            ...
            top_5 = sorted(successful, key=lambda r: r["combined_score"], reverse=True)[
                :5
            ]
