...
training_problems = sorted(
    [d for d in os.listdir(training_dir) if d.startswith("problem_")]
)
...
for problem_dir in training_problems:
    ...
    training_info_path = os.path.join(
        training_dir, problem_dir, "best", "best_program_info.json"
    )
...
print(
    f"Problems missing training score: {sum(1 for r in results if r['training_score'] is None)}"
)
print(
    f"Problems missing testing score: {sum(1 for r in results if r['testing_score'] is None)}"
)
